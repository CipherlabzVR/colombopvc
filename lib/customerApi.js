...
/**
 * GET /ECommerce/GetECommerceCustomerProfileById?customerId=
 * @param {number} customerId
 */
export async function getECommerceCustomerProfileById(customerId) {
  const data = await api.get("/ECommerce/GetECommerceCustomerProfileById", {
    customerId,
  });
  return data;
}
...
/**
 * POST /ECommerce/UpdateECommerceCustomerProfile
 * Email must match the signed-in account (same as stored session).
 * @param {Object} payload
 ...
 * @param {string} payload.lastName
 * @param {string} payload.mobileNo
 * @param {string} [payload.currentPassword] - required if newPassword is set
 * @param {string} [payload.newPassword]
 */
export async function updateECommerceCustomerProfile(payload) {
  const body = {
  ...
  const data = await api.post("/ECommerce/UpdateECommerceCustomerProfile", body);
  return data;
}

/**
 * POST /ECommerce/ForgotECommerceCustomerPassword
 * Sends a password-reset OTP to the customer's email.
 * @param {{ email: string }} payload
 * @returns {Promise<object>}
 */
export async function sendPasswordResetCode(payload) {
  const body = { email: (payload.email ?? "").trim() };
  const data = await api.post("/ECommerce/ForgotECommerceCustomerPassword", body);
  return data;
}

/**
 * POST /ECommerce/VerifyECommerceCustomerOTP
 * Validates the OTP the customer received via email.
...
export async function verifyPasswordResetCode(payload) {
  ...
  const data = await api.post("/ECommerce/VerifyECommerceCustomerOTP", body);
  return data;
}

/**
 * POST /ECommerce/ResetECommerceCustomerPassword
 * Resets the customer password using the verified OTP.
 * @param {{ email: string, otp: string, newPassword: string, confirmPassword: string }} payload
 * @returns {Promise<object>}
 */
export async function resetCustomerPassword(payload) {
  ...
  const data = await api.post("/ECommerce/ResetECommerceCustomerPassword", body);
  return data;
}
