...
/** OrderStatus enum from backend: Queued=1, InProgress=2, Served=3, Completed=4 */
...
/**
 * GET /ECommerce/GetOnlineOrderByOrderIdOrOrderNo?orderIdOrNo={orderIdOrNo}
 * Look up a single order by Order ID or Order No. Works for everyone (signed in or guest).
 * @param {string} orderIdOrNo - Order ID (numeric) or Order No (e.g. invoice number)
 * @returns {Promise<object|null>} Single order (OnlineOrderResponse) or null if not found / API error
 */
export async function getOnlineOrderByOrderIdOrOrderNo(orderIdOrNo) {
  const trimmed = String(orderIdOrNo ?? "").trim();
 ...
 * @param {number} [payload.OrderStatus] - e.g. 0 Queued
