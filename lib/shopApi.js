 ...
 *
 * SortType (ApexflowERP.Domain.Enums.SortType): 1 price asc, 2 price desc, 3 newest first, 4 oldest first. Default 0 → order by Code.
 */

import api, { API_BASE } from "@/lib/base/api";

/** Matches backend `SortType` enum (integer sent in JSON). */
export const WebItemsSortType = Object.freeze({
  PriceLowToHigh: 1,
  PriceHighToLow: 2,
  NewestOnTop: 3,
  OldestOnTop: 4,
...
export async function getAllItemsForWeb(opts = {}) {
  ...
  const data = await api.post("/Items/GetAllItemsForWeb", body, undefined, { signal });
...
/**
 * Newest web-visible items (backend orders by `CreatedOn` desc when sortType = NewestOnTop).
 * Use a page size larger than the display count so the client can skip entirely-OOS rows and still fill a grid.
 *
 * @param {Object} [opts]
 * @param {number} [opts.pageSize=48] — max 100
 * @returns {Promise<{ totalCount: number, items: Array }>}
 */
export async function fetchLatestWebItems(opts = {}) {
  const { pageSize = 48, ...rest } = opts;
  const size = Math.max(1, Math.min(100, pageSize));
  return getAllItemsForWeb({
    pageNumber: 1,
    pageSize: size,
    sortType: WebItemsSortType.NewestOnTop,
    ...rest,
  });
}
