...
export default function ProductQuickView({ product, onClose }) {
  ...
  const [selectedImageIndex, setSelectedImageIndex] = useState(() =>
    getFirstInStockImageIndex(getAllImages(product)),
  );
  const { addToCart } = useCart();
  const { rules, productRules } = useCategoryPromotions();

  const allImages = useMemo(() => getAllImages(product), [product]);
  /** Same purchasable line as grid / detail — covers no-image + API fallback cases where the gallery list is empty. */
  const canonicalCartLine = useMemo(() => productToCartLine(product), [product]);
  ...
  const isVariantSelection = Boolean(currentImage && currentImage.id !== "main");
  const addDisabled =
    !!currentImage?.isOutOfStock ||
    (!isVariantSelection && !canonicalCartLine);
  ...
  function handleAddToCart() {
    ...
    if (isVariant) {
      ...
      if (!canonicalCartLine) return;
      addToCart(canonicalCartLine, qty);
    }
    onClose();
  }
        ...
        <div className="shrink-0 border-t border-slate-200 px-5 sm:px-6 py-4 bg-white rounded-b-xl">
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              ...
              {currentImage?.isOutOfStock
                ? "Out of Stock"
                : addDisabled && !currentImage?.isOutOfStock
                  ? "Unavailable"
                  : "Add to Cart"}
            </button>
          </div>
